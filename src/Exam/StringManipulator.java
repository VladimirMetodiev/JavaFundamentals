...
public class StringManipulator {
    public static void main(String[] args){
        ...
        while(!(expression = input.nextLine()).equals("Done")){
            ...
            switch(command[0]){
                case "Change": data = changesSymbols(data, command[1].charAt(0), command[2]); break;
                case "Includes": includesString(data, command[1]); break;
                case "End": endsWithExpression(data, command[1]); break;
                case "Uppercase": data = uppercaseString(data); break;
                case "FindIndex": findsIndex(data, command[1].charAt(0)); break;
                case "Cut": cutsLetters(data, Integer.parseInt(command[1]), Integer.parseInt(command[2])); break;
                default: break;
            }
        }
    }
...
//Java Fundamentals Final Exam - 03 August 2019
